...
- **komenasne_task_setup.bat** — 5分ごとのバックグラウンド記録タスクを登録/解除します。
  ...
  自動で `kakolog\` に保存されます（ウィンドウは表示されません）。
- **komenasne_recerror.bat** — 録画タイトルの一部を入力すると、部分一致する録画失敗リストを表示します。
- **komenasne_fixrec.bat** — XMLファイル名と分数を指定して過去ログを再取得します（ファイルのドラッグ＆ドロップ対応）。
